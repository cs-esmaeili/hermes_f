...
const TicketDetails = ({ setParentLoading, selectedTicket, setSelectedTicket }) => {
    ...
    const { getTicketByIdRequest } = useGetTicketById((data) => {
        setSelectedTicket(data);
    });

    const { updateTicketRequest } = useUpdateTicket(() => {
        getTicketByIdRequest(selectedTicket._id);
    });
    ...
    return (
        <div className={`flex flex-col gap-4 p-6 bg-primary rounded-lg shadow-lg text-white ${process.env.NEXT_PUBLIC_DIRECTION}`}>
            ...
            <div className="bg-secondary p-3 rounded-lg">
                ...
                <button
                    onClick={() => {
                        updateTicketRequest({ ticket_id: selectedTicket._id, comment: newComment.trim() })
                    }}
